<%-- 
    Document   : add_watchlater
    Created on : 6 Mar, 2022, 9:44:29 PM
    Author     : skbag
--%>
    ...
    if(email==null){
        ...
        if(request.getParameter("code").length()!=0){
            ...
            try{
                ...
                ResultSet rs=st.executeQuery("select * from watch_later where (email='"+email+"' AND videocode='"+videocode+"')");
                if(rs.next()){
                    PreparedStatement ps=cn.prepareStatement("delete from watch_later where (email=? AND videocode=?)");
